...
function ecGeneratePrivate()
{
    ...
    var n1 = n.subtract(BigInteger.ONE);
    var r = new BigInteger(n.bitLength(), rng);
    this.privateKey = r.mod(n1).add(BigInteger.ONE);
}
...
function ecSetPublicHex(pubkey)
{
    var n = this.ec.getN(); 
    ...
    this.publicKey = new ECPointFp(cv, cv.fromBigInteger( bx ), cv.fromBigInteger( by ) );
}
...
function ecSetCipherTextM1Hex(_M1)
{
    var pubObj = JSON.parse(_M1);
    var pub_x_str = pubObj.x; 
    var pub_y_str = pubObj.y;  
    var bx = new BigInteger(pub_x_str, 16); 
    var by = new BigInteger(pub_y_str, 16); 
    var cv = this.ec.getCurve();
    this.M1 = new ECPointFp(cv, cv.fromBigInteger( bx ), cv.fromBigInteger( by ) );
}
...
function ecSetCipherTextHex( jsonStr )
{
    var n = this.ec.getN();  // new BigInteger(document.ecdhtest.n.value);
    // decomposite the signature

    var ctObj = JSON.parse( jsonStr );
    // check the type 
    // ...
    var m1_x_str = ctObj.m1x; 
    var m1_y_str = ctObj.m1y; 
    var m2_str = ctObj.m2; 
    // creating ecpoint 

    var m1x = new BigInteger(m1_x_str, 16); 
    var m1y = new BigInteger(m1_y_str, 16); 

    var cv = this.ec.getCurve();

    this.M1 = new ECPointFp(cv, cv.fromBigInteger( m1x ), cv.fromBigInteger( m1y ) );
    this.M2 = new BigInteger( m2_str,16 );
...
ECAsymCrypto.prototype.SetPublicHex = ecSetPublicHex;
...
ECAsymCrypto.prototype.SetCipherTextHex = ecSetCipherTextHex
