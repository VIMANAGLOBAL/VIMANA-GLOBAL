...
#if 1 // USE LOCAL AES (OPT WORKAR) 20.12.2013 +++
...
const unsigned short gf_mul1x[256] =
{
...
void AddRoundKey(uchar state[][4], unsigned int w[])
{
   uchar subkey[4];
...
uint SubWord(uint word)
{
   ...
   result = (int)aes_sbox[word & 0xFF];
   result += (int)aes_sbox[(word >> 8) & 0xFF] << 8;
   result += (int)aes_sbox[(word >> 16) & 0xFF] << 16;
   result += (int)aes_sbox[(word >> 24) & 0xFF] << 24;
   return(result);
}
...
void KeyExpansion(unsigned char key[], unsigned int w[], int keysize)
{
   ...
   //***********************************
    for(idx=0;idx<60;idx++)
	{
			...
			R12=w[idx]>>8;
			R13=w[idx];
			w[idx]=(R13<<24)+(R12<<16)+(R11<<8)+(R10<<0);
	}
...
void aes_encrypt(unsigned char _in[], unsigned char _out[], unsigned int key[])
{
	uint statein[4];
	unsigned char state[4][4];
    uint* stateic = (uint*)state;

	int i = 8;
    register unsigned long C, L;
	
	unsigned int *in=(unsigned int *)_in;
	...
	{
        // Column 0
		C = aes_sbox[state[0][0]] | (aes_sbox[state[1][1]] << 8) | (aes_sbox[state[2][2]] << 16) | (aes_sbox[state[3][3]] << 24);
		L = gf_mul1x[state[2][2]] ^ C; 
		L = (L >> 8) | (L << 24);
		L ^= gf_mul1x[state[3][3]] ^ C; 
		L = (L >> 8) | (L << 24);
		L ^= gf_mul1x[state[0][0]];
		statein[0] = ((L >> 8) | (L << 24)) ^ gf_mul1x[state[1][1]] ^ *key++;
		
        // Column 1
		C = aes_sbox[state[1][0]] | (aes_sbox[state[2][1]] << 8) | (aes_sbox[state[3][2]] << 16) | (aes_sbox[state[0][3]] << 24);
		L = gf_mul1x[state[3][2]] ^ C; 
		L = (L >> 8) | (L << 24);
		L ^= gf_mul1x[state[0][3]] ^ C; 
		L = (L >> 8) | (L << 24);
		L ^= gf_mul1x[state[1][0]]; 
		statein[1]  = ((L >> 8) | (L << 24)) ^  gf_mul1x[state[2][1]] ^ *key++;
	
        // Column 2
		C = aes_sbox[state[2][0]] | (aes_sbox[state[3][1]] << 8) | (aes_sbox[state[0][2]] << 16) | (aes_sbox[state[1][3]] << 24);
		L = gf_mul1x[state[0][2]] ^ C; 
		L = (L >> 8) | (L << 24);
		L ^= gf_mul1x[state[1][3]] ^ C; 
		L = (L >> 8) | (L << 24);
		L ^= gf_mul1x[state[2][0]]; 
		statein[2]  = ((L >> 8) | (L << 24)) ^ gf_mul1x[state[3][1]] ^ *key++;

        // Column 3
		C = aes_sbox[state[3][0]] | (aes_sbox[state[0][1]] << 8) | (aes_sbox[state[1][2]] << 16) | (aes_sbox[state[2][3]] << 24);
		L = gf_mul1x[state[1][2]] ^ C; 
		L = (L >> 8) | (L << 24);
		L ^= gf_mul1x[state[2][3]] ^ C; 
		L = (L >> 8) | (L << 24);
		L ^= gf_mul1x[state[3][0]]; 
		stateic[3]  = ((L >> 8) | (L << 24)) ^ gf_mul1x[state[0][1]] ^ *key++;
	...
	out[0] = (aes_sbox[state[0][0]] | (aes_sbox[state[1][1]]<<8) | (aes_sbox[state[2][2]]<<16) | (aes_sbox[state[3][3]]<<24)) ^ *key++;
	out[1] = (aes_sbox[state[1][0]] | (aes_sbox[state[2][1]]<<8) | (aes_sbox[state[3][2]]<<16) | (aes_sbox[state[0][3]]<<24)) ^ *key++;
	out[2] = (aes_sbox[state[2][0]] | (aes_sbox[state[3][1]]<<8) | (aes_sbox[state[0][2]]<<16) | (aes_sbox[state[1][3]]<<24)) ^ *key++;
	out[3] = (aes_sbox[state[3][0]] | (aes_sbox[state[0][1]]<<8) | (aes_sbox[state[1][2]]<<16) | (aes_sbox[state[2][3]]<<24)) ^ *key++;
}

void aes_decrypt(unsigned char in[], unsigned char out[], unsigned int key[])
{
   unsigned char state[4][4];
...
unsigned int key_scheduleAES[60];
unsigned char IVEC_AES[16];


unsigned int Tmp_key_scheduleAES[60];
unsigned char Tmp_IVEC_AES[16];


void reset_ivec_aes(void)
{
  memset(IVEC_AES,0x11, 16);
}

void AESInit(unsigned char *KeyENC)
{
	memcpy			(IVEC_AES,	KeyENC+16,			16);
	KeyExpansion(KeyENC,		key_scheduleAES,128);
}
...
void AESCbcDecrypt(unsigned char *Buf, int Len)
{
	...
	unsigned int *p1=(unsigned int *)Buf;
	unsigned int *p2=(unsigned int *)IVEC_AES;;

	for(i=0;i<Block;i++)
	{
		memcpy(ciphertext,Buf+(i*16),16);
		
		aes_decrypt(ciphertext,Buf+(i*16),key_scheduleAES); 
...
void AESCbcEncrypt(unsigned char *Buf, int Len)
{
	...
	for(i=0;i<Block;i++)
	{
		memcpy(plaintext,Buf+(i*16),16);
		
		for(j=0;j<16;j++) plaintext[j]^=IVEC_AES[j];

		aes_encrypt(plaintext,ciphertext,key_scheduleAES); 
		
		memcpy(IVEC_AES,	ciphertext, 16);
		memcpy(Buf+(i*16),	ciphertext, 16);
...
void AESOfb(unsigned char *Buf, int Len)
{
	...
	for(i=0;i<Block;i++)
	{
		memcpy(plaintext,IVEC_AES,16);	
		aes_encrypt(plaintext,ciphertext,key_scheduleAES); 
		memcpy(plaintext,Buf+(i*16), 16);
		
		for(j=0;j<16;j++) plaintext[j]^=ciphertext[j];

		
		memcpy(IVEC_AES,		ciphertext, 16);
		memcpy(Buf+(i*16),	plaintext, 16);
...
unsigned char IVEC_MAC[16];
unsigned int key_scheduleMAC[60]; //was 60
...
{
	memset(IVEC_MAC,0x22,16);
	KeyExpansion(KeyENC,key_scheduleMAC,128);
}

//*********************************************

void MACAESOfbEncrypt(int Len)
{
	unsigned char plaintext[16];
	unsigned char ciphertext[16];
	int i;
	...
	for(i=0;i<Block;i++)
	{
		memcpy(plaintext,IVEC_MAC,16);
		aes_encrypt(plaintext,ciphertext,key_scheduleMAC); 
		memcpy(IVEC_MAC,	ciphertext, 16);		
	}
...
void MAC_OFB_Calc(unsigned char *InBuf, int Len, unsigned char *key, unsigned char *OutBuf)
{
  unsigned char Buf[16];
  unsigned char Ivec[16];

  int KolBlock8=Len/16;
  int KolBlock7=Len%16;
  int Pos;
  ...
  {
    memset(Buf,0x00, sizeof(Buf));
    memcpy(Buf,InBuf+Pos,KolBlock7);
    MACAESOfbEncrypt(16);//
  }

  memcpy(OutBuf,IVEC_MAC,8);// возвращаем только 8 байта
}
//------------------------------------------------------------------------------

void MAC_AES_CalcBlockInit(unsigned char *key,unsigned char *Ivec)
{
  MACInit(key);
}
...
void MAC_OFB_CalcBlock(unsigned char *InBuf, int Len, unsigned char *mac)
{
  unsigned char Buf[16];
...
void AES_ENCRYPT_BLOCK(unsigned char *InBuf, int Len)
{
  ...
  if (KolBlock7)
  {
    memset(Buf,0x00, sizeof(Buf));
    memcpy(Buf,InBuf+Pos,KolBlock7);
    AESCbcEncrypt(Buf, 16);//
		memcpy(InBuf+Pos,Buf,KolBlock7);
  }
...
void AesGetToTemp()
{
	memcpy(Tmp_key_scheduleAES,key_scheduleAES,240);
	memcpy(Tmp_IVEC_AES,IVEC_AES,16);
}

void AesSetToTemp()
{
	memcpy(key_scheduleAES,Tmp_key_scheduleAES,240);
	memcpy(IVEC_AES,Tmp_IVEC_AES,16);
}
//*********************************************************************************************
